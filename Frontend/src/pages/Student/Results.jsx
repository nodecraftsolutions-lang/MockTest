...
const Results = () => {
  ...
  return (
    <ResponsiveContainer className="space-y-6 pb-12">
      {/* Header */}
      <motion.div 
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="bg-gradient-to-r from-primary-600 to-primary-800 text-white rounded-2xl p-6 shadow-lg"
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold mb-2">Test Results Dashboard</h1>
            <p className="text-primary-100 text-sm md:text-base">Track your performance and progress across all tests</p>
          </div>
          <div>
            <button 
              onClick={handleRefresh}
              className="flex items-center bg-white/20 hover:bg-white/30 text-white px-3 py-2 rounded-lg transition-all text-sm md:text-base"
            ...
            >
              ...
              <span className="whitespace-nowrap">{isRefreshing ? 'Refreshing...' : 'Refresh Data'}</span>
            </button>
          </div>
        </div>
      </motion.div>
      ...
      <ResponsiveGrid cols={1} colsSm={2} colsMd={3} colsLg={6} gap={4}>
        {[
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            ...
            className="flex items-center justify-center md:justify-start px-3 py-2 md:px-4 md:py-2.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-700 transition-colors text-sm md:text-base"
          >
            <Filter className="w-4 h-4 md:w-5 md:h-5 mr-1 md:mr-2" />
            <span>Filters</span>
            {showFilters ? (
              <ChevronUp className="w-4 h-4 ml-1 md:ml-2" />
            ) : (
            ...
            <ResponsiveGrid cols={1} colsMd={3} gap={4}>
              <div>
                ...
                <select 
                  ...
                  className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm"
                >
              ...
              <div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between border border-gray-300 rounded-lg p-2 bg-white hover:bg-gray-50 text-sm"
                >
                  <span className="text-sm">{sortOrder === 'desc' ? 'Highest to Lowest' : 'Lowest to Highest'}</span>
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    ...
                    <div>
                      {attempt.rank && (
                        <div className="flex items-center bg-yellow-50 text-yellow-700 px-2 py-1 rounded-md text-xs mb-1">
                          <Trophy className="w-3 h-3 mr-1 text-yellow-600" /> Rank {attempt.rank}
                        </div>
                      )}
                      {attempt.percentile && (
                        <div className="flex items-center bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md text-xs">
                          <BarChart3 className="w-3 h-3 mr-1 text-indigo-600" /> {attempt.percentile}%ile
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-3 text-xs text-gray-600">
                    ...
                    {actualTime !== null && (
                      <div className="flex items-center bg-gray-50 px-2 py-0.5 rounded">
                  ...
                  <div className="flex flex-col gap-2">
                    {(attempt.status === 'submitted' || attempt.status === 'auto-submitted') && (
                      <Link
                        ...
                        className="text-primary-600 hover:text-primary-900 flex items-center justify-center bg-primary-50 hover:bg-primary-100 px-3 py-2 rounded-lg transition-colors text-sm"
                      >
                    ...
                    {attempt.status === 'in-progress' && (
                      <Link
                        ...
                        className="text-blue-600 hover:text-blue-900 flex items-center justify-center bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-lg transition-colors text-sm"
                      >
...
const StatCard = ({ icon: Icon, color, value, label }) => (
  <div className={`bg-white rounded-xl shadow-md p-4 border-b-4 border-${color}-500 hover:shadow-lg transition-shadow`}>
    <div className={`w-10 h-10 bg-${color}-100 text-${color}-600 rounded-lg flex items-center justify-center mb-2`}>
